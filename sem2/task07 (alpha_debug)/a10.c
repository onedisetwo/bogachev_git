...
int main(int argc, char *argv[]){
	const int TASK = 10, MIN_ARGC = 7, MAX_ARGC = 9;
	const char *VAR_LIST = "t m n p kA (nameA) kX0 (nameX0)";

	int m, n, p, kA, kX0, ret;
	char *nameA, *nameX0;
	int  arg, retA, retX0;
	double *a, *x0, *x1, *b, *r, *w, t, time, r1, r2;

	arg = 1;
	if( !( 	argc >= MIN_ARGC && argc <= MAX_ARGC
		&& sscanf(argv[arg++], "%lf", &t) == 1
	...
	time = clock();
	ret = solve10(a, b, x0, x1, r, w, n, m, t);
	time = (clock() - time) / CLOCKS_PER_SEC;
	if(ret == SUCCESS){
		printf("Vector x_m:\n");
		print_matrix(x0, 1, n, p);
		r1 = get_r1_task2_10(a, x0, b, n);
		r2 = get_r2_task2_10(x0, n);
